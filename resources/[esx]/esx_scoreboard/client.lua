...
function UpdatePlayerTable(connectedPlayers)
	...
	for k,v in pairs(connectedPlayers) do
		table.insert(formattedPlayerList, ('<tr><td>%s</td><td class="pid">%s</td><td class="sjob">%s<td class="sjobgrade">%s</td><td class="ping">%s</td></tr>'):format(v.name, v.id, v.jobLabel,v.jobgradeLabel, v.ping))
		players = players + 1
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F10']) and IsInputDisabled(0) then
			ToggleScoreBoard()
			Citizen.Wait(200)
		elseif IsControlJustReleased(0, Keys['F10']) and IsInputDisabled(0) then
			SendNUIMessage({
				action  = 'close'
			})
		end
