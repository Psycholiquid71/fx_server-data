...
Citizen.CreateThread(function()
    RequestModel(GetHashKey("CS_Magenta"))
	
    while not HasModelLoaded(GetHashKey("CS_Magenta")) do
        Wait(1)
    end
	
	if Config.EnableHospital then
		for _, item in pairs(Config.Locations23) do
			local npc = CreatePed(4, 0x5816C61A, item.x, item.y, item.z, item.heading, false, true)
	        ...
	        Citizen.Wait(200)	
			TaskPlayAnim(npc,"cellphone@str","f_cellphone_call_listen_b",1.0, 1.0, -1, 9, 1.0, 0, 0, 0)
			--TaskPlayAnim(ped, animDictionary, animationName, speed, speedMultiplier, duration, flag, playbackRate, lockX, lockY, lockZ)--
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableHospital then
		for _, item in pairs(Config.Locations24) do
			local npc = CreatePed(4, 0xAB594AB6, item.x, item.y, item.z, item.heading, false, true)
...
Citizen.CreateThread(function()
	...
	if Config.EnableHospital then
		for _, item in pairs(Config.Locations26) do
			local npc = CreatePed(4, 0xB6AA85CE, item.x, item.y, item.z, item.heading, false, true)
