...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 73) and callActive then
            ...
            SendNotification("You accepted the phone call")
        ...
        elseif IsControlJustPressed(1, 249) and callActive then
            SendNotification("You declined the phone call")
            callActive = false
        end
        if haveTarget then
...
AddEventHandler("call:callIncoming", function(job, pos)
    ...
    work = job
    target.pos = pos
    SendNotification("Press ~g~X~s~ to accept the call or ~g~N~s~ to decline it")
end)
...
AddEventHandler("target:call:taken", function(taken)
    if taken == 1 then
        SendNotification("Someone is on their way")
    elseif taken == 0 then
        SendNotification("Nobody is coming, sorry")
    elseif taken == 2 then
        SendNotification("Please try again in a moment")
    end
end)
